...
public class AccountController {
	 ...
	 * //SaveALL()
	 * 
	 * @PostMapping("/addAll") public ResponseEntity<?> addListAccount(@RequestBody
	 * List<Account> acc) throws NoDataException { List<Account> acount=new
	 * ArrayList<Account>(); List<Account> list = (List<Account>)
	 * accountService.addListAccount(acount).stream().distinct().collect(Collectors.
	 * toList()); if (list != null) { return new
	 * ResponseEntity<>("Added successfully", HttpStatus.CREATED); } else { throw
	 * new NoDataException("Empty data"); } }
	 */
...
//	Select * from customer;
	...
	//SaveALL()
	@PostMapping("/addAllList")
	public ResponseEntity<Object> addListAccount(@RequestBody List<Account> acc) throws NoDataException {
	//	List<Account> acountDetailsList=new  ArrayList<Account>();
		List<Account> msg = (List<Account>) accountService.addListAccount(acc);//.stream().distinct().collect(Collectors.toList());
		List<Account> acountDetailsList=new  ArrayList<>();
		for (Account accountss:msg) {
			Account acco=new  Account();
			acco.setAccountNumber(accountss.getAccountNumber());
			acco.setAccountType(accountss.getAccountType());
			acco.setBalanceAmount(accountss.getBalanceAmount());
			acco.setCustomer(accountss.getCustomer());
			
			//acco.add(acountDetails);
			acountDetailsList.add( acco);
			acountDetailsList=acountDetailsList.stream().distinct().collect(Collectors.toList());
			//list.add((Account) acountDetailsList);
		}
